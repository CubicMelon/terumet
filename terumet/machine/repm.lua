...
function base_repm.process(repm, dt)
    ...
    if base_mach.has_upgrade(repm, 'speed_up') then dt = dt * 2 end

    if repm.state == base_repm.STATE.RMAT_MELT then
        local rmat_time = math.min(repm.state_time, dt)
        local heat_req = rmat_time * opts.MELT_HUPS
        if base_mach.expend_heat(repm, heat_req, 'Melting repair material') then
    ...
    elseif repm.state == base_repm.STATE.REPAIRING then
        ...
        if rep_item_wear > 0 and base_mach.expend_heat(repm, dt * opts.REPAIR_HEAT, 'Repairing') then
            ...
            if item_full_repair_cost then
                ...
                local rmp_used = math.ceil(math.min(opts.REPAIR_RATE, rep_item_wear * repair_per_rmp))
